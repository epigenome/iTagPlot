...
public class DoubleSliderSkin extends BehaviorSkinBase<DoubleSlider, DoubleSliderBehavior>{
...
//				tickLine.setTickLabelFill(Color.WHITE);
	...
	void positionThumb() {
		...
		double ly1 = (horizontal) ? thumbTop :
			this.getSkinnable().getInsets().getTop() + trackLength - (trackLength * ((s.getValue1() - s.getMin()) /
		...
		double ly2 = (horizontal) ? thumbTop :
			this.getSkinnable().getInsets().getTop() + trackLength - (trackLength * ((s.getValue2() - s.getMin()) /
					(s.getMax() - s.getMin()))) - thumbHeight / 4; 
	...
	@Override protected double computeMinWidth(double height, double topInset, double rightInset, double bottomInset, double leftInset) {
		if (getSkinnable().getOrientation() == Orientation.HORIZONTAL) {
			return (leftInset + minTrackLength() + thumb1.minWidth(-1) + rightInset);
		} else {
			return(leftInset + thumb1.prefWidth(-1) + rightInset);
		}
	}

	@Override protected double computeMinHeight(double width, double topInset, double rightInset, double bottomInset, double leftInset) {
		if (getSkinnable().getOrientation() == Orientation.HORIZONTAL) {
			return(topInset + thumb1.prefHeight(-1) + bottomInset);
		} else {
			return(topInset + minTrackLength() + thumb1.prefHeight(-1) + bottomInset);
		}
	}
	...
	@Override protected double computePrefWidth(double height, double topInset, double rightInset, double bottomInset, double leftInset) {
		if (getSkinnable().getOrientation() == Orientation.HORIZONTAL) {
			...
			return (leftInset) + Math.max(thumb1.prefWidth(-1), track.prefWidth(-1)) +
	...
	@Override protected double computePrefHeight(double width, double topInset, double rightInset, double bottomInset, double leftInset) {
		if (getSkinnable().getOrientation() == Orientation.HORIZONTAL) {
			return topInset + Math.max(thumb1.prefHeight(-1), track.prefHeight(-1)) +
